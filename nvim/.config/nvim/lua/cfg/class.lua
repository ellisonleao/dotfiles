...
--
-- local v = Vec3(1, 2, 3)
BaseType.__members.__init = function()
end
...
local function new_class(args, strict)
  -- Arg checking
  if #args == 0 then
    error("class takes at least one arg", 2)
  end
  for _, v in pairs(args) do
    if type(v) ~= "table" then
      error("class args must all be tables", 2)
    end
  end
